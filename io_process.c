...
static void resolveConflict(char *dst_path, size_t dst_size) {
  if (sceIoGetstat(dst_path, NULL) < 0) return;
  char base[MAX_PATH_LENGTH], ext[MAX_PATH_LENGTH];
  char *p = strrchr(dst_path, '.');
  if (p && p > strrchr(dst_path, '/')) {
    strncpy(ext, p, MAX_PATH_LENGTH - 1);
    ext[MAX_PATH_LENGTH - 1] = '\0';
    *p = '\0';
  } else {
    ext[0] = '\0';
  }
  strncpy(base, dst_path, MAX_PATH_LENGTH - 1);
  base[MAX_PATH_LENGTH - 1] = '\0';
  int n = 1;
  while (n < 1000) {
    if (ext[0])
      snprintf(dst_path, dst_size, "%s (%d)%s", base, n, ext);
    else
      snprintf(dst_path, dst_size, "%s (%d)", base, n);
    if (sceIoGetstat(dst_path, NULL) < 0) return;
    n++;
  }
}
...
int copy_thread(SceSize args_size, CopyArguments *args) {
  ...
  if (args->copy_mode == COPY_MODE_MOVE && !diff_partition) { // Move
    ...
    for (i = 0; i < args->copy_list->length; i++) {
      ...
      resolveConflict(dst_path, sizeof(dst_path));
