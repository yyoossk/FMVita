    ...
    if (sort != SORT_NONE) {
      ...
      while (p) {
        ...
        if (sort == SORT_BY_NAME) {
          ...
          if (entry->is_folder == p->is_folder) {
            SceDateTime e_mtime = entry->mtime, p_mtime = p->mtime;
            if (e_mtime.year < 1900 || e_mtime.year > 9999) { e_mtime.year = 1970; e_mtime.month = 1; e_mtime.day = 1; e_mtime.hour = 0; e_mtime.minute = 0; e_mtime.second = 0; e_mtime.microsecond = 0; }
            if (p_mtime.year < 1900 || p_mtime.year > 9999) { p_mtime.year = 1970; p_mtime.month = 1; p_mtime.day = 1; p_mtime.hour = 0; p_mtime.minute = 0; p_mtime.second = 0; p_mtime.microsecond = 0; }

            int cmp = 0;
            if (e_mtime.year != p_mtime.year) cmp = (e_mtime.year > p_mtime.year) ? 1 : -1;
            else if (e_mtime.month != p_mtime.month) cmp = (e_mtime.month > p_mtime.month) ? 1 : -1;
            else if (e_mtime.day != p_mtime.day) cmp = (e_mtime.day > p_mtime.day) ? 1 : -1;
            else if (e_mtime.hour != p_mtime.hour) cmp = (e_mtime.hour > p_mtime.hour) ? 1 : -1;
            else if (e_mtime.minute != p_mtime.minute) cmp = (e_mtime.minute > p_mtime.minute) ? 1 : -1;
            else if (e_mtime.second != p_mtime.second) cmp = (e_mtime.second > p_mtime.second) ? 1 : -1;
            else if (e_mtime.microsecond != p_mtime.microsecond) cmp = (e_mtime.microsecond > p_mtime.microsecond) ? 1 : -1;

            // Sort by date within the same type (newest first)
            if (cmp > 0)
              break;
