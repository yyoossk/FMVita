...
// Touch confirm callbacks (replace YESNO system dialogs)

static void refreshLiveareaYes() {
...
MenuEntry menu_main_entries[] = {
  ...
  { DELETE,         5, 0, CTX_INVISIBLE },
  { RENAME,         6, 0, CTX_INVISIBLE },
  { PROPERTIES,     7, 0, CTX_INVISIBLE },
  { NEW,            8, CTX_FLAG_MORE, CTX_VISIBLE },
  { SORT_BY,        9, CTX_FLAG_MORE, CTX_VISIBLE },
  { MORE,           10, CTX_FLAG_MORE, CTX_INVISIBLE },
...
MenuEntry menu_adhoc_entries[] = {
  { SEND,    11, 0, CTX_INVISIBLE },
  { RECEIVE, 12, 0, CTX_INVISIBLE },
};
...
MenuEntry menu_more_entries[] = {
  { COMPRESS,       10, 0, CTX_INVISIBLE },
  { INSTALL_ALL,    11, 0, CTX_INVISIBLE },
  { INSTALL_FOLDER, 12, 0, CTX_INVISIBLE },
  { EXPORT_MEDIA,   13, 0, CTX_INVISIBLE },
  { CALCULATE_SHA1, 14, 0, CTX_INVISIBLE },
};
...
MenuEntry menu_new_entries[] = {
  { NEW_FILE,      8, 0, CTX_INVISIBLE },
  { NEW_FOLDER,    9, 0, CTX_INVISIBLE },
  { BOOKMARKS_NEW, 10, 0, CTX_INVISIBLE },
};
...
int contextMenuMoreEnterCallback(int sel, void *context);
int contextMenuBookmarksEnterCallback(int sel, void *context);
int contextMenuAdhocEnterCallback(int sel, void *context);
int contextMenuNewEnterCallback(int sel, void *context);
...
void setContextMenuMainVisibilities() {
  ...
  // Undo visible only when operation available
  if (!undo_available) {
    menu_main_entries[MENU_MAIN_ENTRY_UNDO].visibility = CTX_INVISIBLE;
  }

  // Invisible if it's not folder or not a known encrypted partition folder
  if (!file_entry->is_folder) {
    menu_main_entries[MENU_MAIN_ENTRY_OPEN_DECRYPTED].visibility = CTX_INVISIBLE;
  } else {
    char target_path[MAX_PATH_LENGTH];
    snprintf(target_path, sizeof(target_path), "%s%s", file_list.path, file_entry->name);
    
    int is_encrypted = 0;
    if (strcasestr(target_path, "/app/") || strcasestr(target_path, ":app/") ||
        strcasestr(target_path, "/patch/") || strcasestr(target_path, ":patch/") ||
        strcasestr(target_path, "/addcont/") || strcasestr(target_path, ":addcont/") ||
        strcasestr(target_path, "/appmeta/") || strcasestr(target_path, ":appmeta/") ||
        strcasestr(target_path, "/user/") || strcasestr(target_path, ":user/")) {
      is_encrypted = 1;
    } else {
      int len = strlen(target_path);
      if (len >= 4 && (strcasecmp(target_path + len - 4, "/app") == 0 || strcasecmp(target_path + len - 4, ":app") == 0)) is_encrypted = 1;
      else if (len >= 6 && (strcasecmp(target_path + len - 6, "/patch") == 0 || strcasecmp(target_path + len - 6, ":patch") == 0)) is_encrypted = 1;
      else if (len >= 8 && (strcasecmp(target_path + len - 8, "/addcont") == 0 || strcasecmp(target_path + len - 8, ":addcont") == 0)) is_encrypted = 1;
      else if (len >= 8 && (strcasecmp(target_path + len - 8, "/appmeta") == 0 || strcasecmp(target_path + len - 8, ":appmeta") == 0)) is_encrypted = 1;
      else if (len >= 5 && (strcasecmp(target_path + len - 5, "/user") == 0 || strcasecmp(target_path + len - 5, ":user") == 0)) is_encrypted = 1;
    }
...
void setContextMenuMoreVisibilities() {
  ...
  if (!file_entry->is_folder &&
    ...
    file_entry->type != FILE_TYPE_PNG && file_entry->type != FILE_TYPE_GIF &&
    file_entry->type != FILE_TYPE_MP3 &&
  ...
  switch (sel) {
    case MENU_HOME_ENTRY_REFRESH_LIVEAREA:
    {
      if (is_safe_mode) {
        ...
        setTouchConfirm(language_container[REFRESH_LIVEAREA_QUESTION], refreshLiveareaYes, refreshLiveareaNo);
      }
    ...
    case MENU_HOME_ENTRY_REFRESH_LICENSE_DB:
    {
      if (is_safe_mode) {
        ...
        setTouchConfirm(language_container[REFRESH_LICENSE_DB_QUESTION], refreshLicenseDbYes, refreshLicenseDbNo);
      }
  ...
  switch (sel) {
    ...
    case MENU_MAIN_ENTRY_COPY:
    {
      ...
      if (file_entry) {
        ...
        snprintf(confirm_msg, sizeof(confirm_msg), message, copy_list.length);
        setTouchConfirm(confirm_msg, touchInfoOk, touchInfoOk);
      }
    ...
    case MENU_MAIN_ENTRY_DELETE:
    {
      ...
      if (file_entry) {
        char *message;
        int is_folder = file_entry->is_folder;
        ...
        strncpy(delete_confirm_message, message, 511);
        delete_confirm_message[511] = '\0';
        delete_confirm_is_folder = is_folder;
        setDialogStep(DIALOG_STEP_DELETE_CONFIRM_TOUCH);
      }
...
int contextMenuMoreEnterCallback(int sel, void *context) {
  switch (sel) {
    ...
    case MENU_MORE_ENTRY_INSTALL_ALL:
    {
      ...
      setTouchConfirm(language_container[INSTALL_ALL_QUESTION], installAllYes, installAllNo);
    ...
    case MENU_MORE_ENTRY_INSTALL_FOLDER:
    {
      ...
      if (file_entry) {
        snprintf(cur_file, MAX_PATH_LENGTH, "%s%s", file_list.path, file_entry->name);
        setTouchConfirm(language_container[INSTALL_FOLDER_QUESTION], installAllYes, installAllNo);
      }
    ...
    case MENU_MORE_ENTRY_EXPORT_MEDIA:
    {
      ...
      if (file_entry) {
        ...
        setTouchConfirm(message, exportMediaYes, exportMediaNo);
      }
    ...
    {
      setTouchConfirm(language_container[HASH_FILE_QUESTION], hashFileYes, hashFileNo);
      break;
    }
  }
    ...
    case MENU_NEW_BOOKMARK: {
      ...
      if (file_entry) {
        ...
        if ((res = createSymLink(target, cur_file)) < 0) {
          ...
          setTouchConfirm(language_container[BOOKMARK_CREATED], touchInfoOk, touchInfoOk);
        }
      } else {
        errorDialog(-2);
      }
