...
void drawPropertyDialog() {
  ...
  if (property_dialog.status == PROPERTY_DIALOG_OPENED) {
    ...
    int ok_btn = (enter_button == SCE_SYSTEM_PARAM_ENTER_BUTTON_CIRCLE) ? BUTTON_CIRCLE : BUTTON_CROSS;
    const char *ok_label = language_container[OK];
    float total_w = BUTTON_SIZE + 6 + pgf_text_width(ok_label);
    float bx = (SCREEN_WIDTH - total_w) / 2.0f;
    drawButton(ok_btn, bx, string_y + FONT_Y_SPACE + 1);
    pgf_draw_text(bx + BUTTON_SIZE + 6, string_y + FONT_Y_SPACE, DIALOG_COLOR, ok_label);
  }
}
