...
void readPad() {
  ...
  memset(&touch_back, 0, sizeof(SceTouchData));
...
int vshIoMount(SceVshMountId id, const char *path, int permission, int a4, int a5, int a6) {
  uint32_t buf[3];
...
int isProtectedPath(const char *path) {
  ...
  for (int i = 0; i < sizeof(protected_dirs)/sizeof(protected_dirs[0]); i++) {
      if (strcasecmp(temp, protected_dirs[i]) == 0) return 1;
  }
  return 0;
}
