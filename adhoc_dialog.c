...
void drawAdhocDialog() {
  ...
  if (adhoc_dialog.status == ADHOC_DIALOG_OPENED) {
    ...
    const char *ok_label = language_container[OK];
    const char *cancel_label = language_container[CANCEL];
    int btn_gap = 8, label_gap = 6;
    float total_w = BUTTON_SIZE + label_gap + pgf_text_width(ok_label) + btn_gap + BUTTON_SIZE + label_gap + pgf_text_width(cancel_label);
    float bx = (SCREEN_WIDTH - total_w) / 2.0f;
    float by = string_y + FONT_Y_SPACE;
    drawButton(enter_btn, bx, by + 1);
    pgf_draw_text(bx + BUTTON_SIZE + label_gap, by, DIALOG_COLOR, ok_label);
    bx += BUTTON_SIZE + label_gap + pgf_text_width(ok_label) + btn_gap;
    drawButton(cancel_btn, bx, by + 1);
    pgf_draw_text(bx + BUTTON_SIZE + label_gap, by, DIALOG_COLOR, cancel_label);
  }
}
